...
struct matrix q_rec_now,q_rec_new,q_rec_old;
struct matrix mem_q_rec_x,mem_q_rec_z;
...
/*banana-doughnut of velocity for cfwi*/
...
/*rabbit-ear of velocity for rfwi*/
void rabbit_ear(int isrc,
		struct matrix sub_vp,struct matrix sub_pe,struct matrix sub_wave,
		struct matrix sub_ck_p_now,struct matrix sub_ck_p_old,
		struct matrix sub_p_t,struct matrix sub_p_b,struct matrix sub_p_l,struct matrix sub_p_r,
		struct matrix sub_ck_q_now,struct matrix sub_ck_q_old,
		struct matrix sub_q_t,struct matrix sub_q_b,struct matrix sub_q_l,struct matrix sub_q_r,
		struct matrix sub_p_ajs,struct matrix sub_p_src_sig,struct matrix sub_p_rec_sig);
...
/*least-square reverse time migration*/
void least_square_migration(int lsm_niter,float p1,float p2,float p3,float p4,
			    struct matrix sub_vp,struct matrix sub_wave,struct matrix lsm_refl);

/*velocity kernel for cfwi*/
float cfwi_velo_kernel(int sub_ifreq,int sub_iiter,
		       struct matrix sub_vp,struct matrix sub_wave,struct matrix sub_p);

/*velocity kernel for rfwi*/
float rfwi_velo_kernel(int sub_ifreq,int sub_iiter,float p1,float p2,float p3,float p4,
		       struct matrix sub_vp,struct matrix sub_pe,struct matrix sub_wave,struct matrix sub_p);

/*update velocity model for cfwi*/
...
/*update velocity model for rfwi*/
void update_model_for_rfwi(int sub_ifreq,int sub_iiter,float resi_value,
			   struct matrix sub_vp,struct matrix sub_pe,struct matrix sub_wave,
			   struct matrix gradient,struct matrix sub_gk0,struct matrix sub_dk0);
